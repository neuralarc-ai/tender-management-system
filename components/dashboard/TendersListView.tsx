...
export function TendersListView({ tenders, onSelect }: { tenders: Tender[], onSelect: (id: string) => void }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* Header Stats Area */}
      <div>
        <h2 className="text-3xl font-bold text-neural mb-6">Tenders</h2>
        ...
        <div className="flex gap-4 items-center mb-8">
            ...
            <div className="bg-white rounded-full p-1 pr-6 flex items-center gap-4 shadow-sm border border-gray-100">
                <div className="flex items-center">
                    <div className="bg-neural text-white px-6 py-3 rounded-full text-sm font-medium">
                        Open
                    </div>
                    <div className="bg-aurora text-neural px-6 py-3 rounded-r-full -ml-4 z-0 pl-8 text-sm font-medium">
                        Analyzed
                    </div>
                </div>
      ...
      {/* Main Table Card */}
      <Card className="bg-white rounded-[40px] p-2 shadow-sm border-none">
        <div className="bg-white rounded-[32px] p-6 min-h-[600px]">
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
                ...
                <div className="flex items-center gap-3 ml-auto">
                    ...
                    <Button size="icon" className="rounded-full bg-aurora text-neural hover:bg-aurora-dark w-10 h-10 shadow-sm">
                        <RiAddLine size={20} />
                    </Button>
                    ...
                    <thead>
                        <tr className="text-gray-400 text-xs uppercase tracking-wider border-b border-gray-100 border-dashed">
                            <th className="pb-4 pl-4 w-12">
                                <div 
                                    className={`w-5 h-5 rounded-[6px] border-2 cursor-pointer flex items-center justify-center transition-colors ${selectedIds.size === tenders.length && tenders.length > 0 ? 'bg-neural border-neural' : 'border-gray-300'}`}
                                    onClick={toggleAll}
                                >
                            ...
                            return (
                                <tr 
                                    ...
                                    className={`
                                        ...
                                        ${isSelected ? 'bg-aurora shadow-sm' : 'hover:bg-gray-50'}
                                    `}
                                ...
                                >
                                    ...
                                    <td className={`py-4 pl-4 rounded-l-2xl ${isSelected ? '' : ''}`}>
                                        <div 
                                            className={`w-5 h-5 rounded-[6px] border-2 cursor-pointer flex items-center justify-center transition-colors ${isSelected ? 'bg-neural border-neural' : 'border-gray-300'}`}
                                    ...
                                    <td className="py-4 font-medium text-neural">
                                    ...
                                    <td className="py-4 text-gray-600">
                                        <div className="flex items-center gap-1">
                                            <div className="w-4 h-3 rounded-[2px] bg-drift/20 overflow-hidden relative">
                                                <div className="absolute top-0 bottom-0 left-0 w-1.5 bg-passion"></div>
                                            </div>
                                            Remote
                                        </div>
                                    </td>
                                    
                                    <td className="py-4 text-neural font-medium">
                                        $120,000
                                    </td>
                                    ...
                                    <td className={`py-4 rounded-r-2xl ${isSelected ? '' : ''}`}>
                                        <div className="flex items-center gap-1.5">
                                            <div className={`w-2 h-2 rounded-full ${tender.status === 'open' ? 'bg-verdant' : 'bg-gray-400'}`}></div>
